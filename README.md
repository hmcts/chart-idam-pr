...
Below is example configuration for running this chart on a PR.
...
The `SERVICE_FQDN` is provided by the pipeline, but require you to pass them through.
...
idam-pr:
  service:
    name: divorce
    redirect_uri: ${SERVICE_FQDN}/oauth2/redirect
...
| `dataStoreApi.authorisedServices`              |  A list of services allowed to contact data store api | See [values.yaml](idam-pr/values.yaml) (too many to list)|
